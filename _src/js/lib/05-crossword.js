...
(function($){
  $.fn.crossword = function(entryData) {
    ...
    this.parents().eq(1).after('<div id="puzzle-clues"><div class="puzzle-clues-across"><h2>Across</h2><ol id="across"></ol></div><div class="puzzle-clues-down"><h2>Down</h2><ol id="down"></ol></div><div class="crossword-close"><button type="button" class="btn btn-primary js-clue-close">Close</button></div></div>');
    ...
    var util = {
      highlightEntry: function() {
        ...
        $actives.eq(0).focus();
        $('td.position-' + activePosition).addClass('checked');
        $actives.eq(0).select();
      },
      ...
      getActivePositionFromClassGroup: function(el){
        var classes, e1Ori, e2Ori, e1Cell, e2Cell;

        classes = util.getClasses($(el).parent(), 'position');

        if(classes.length > 1){
          // get orientation for each reported position
          e1Ori = clueLiEls.filter('[data-position=' + classes[0].split('-')[1] + ']').parent().prop('id');
          e2Ori = clueLiEls.filter('[data-position=' + classes[1].split('-')[1] + ']').parent().prop('id');
